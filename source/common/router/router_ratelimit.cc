...
bool GenericValueRateLimitOverride::populateOverride(
    RateLimit::Descriptor& descriptor, const envoy::config::core::v3::Metadata*) const {
  descriptor.limit_.emplace(RateLimit::RateLimitOverride{limit_, unit_});
  return true;
}
...
RateLimitPolicyEntryImpl::RateLimitPolicyEntryImpl(
  ...
  if (config.has_limit()) {
    ...
    case envoy::config::route::v3::RateLimit_Override::OverrideSpecifierCase::kGenericValue:
      limit_override_.emplace(new GenericValueRateLimitOverride(config.limit().generic_value()));
      break;
